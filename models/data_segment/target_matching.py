...
class TargetSegmentor(object):
    def __init__(self, cfg, data: Dict[str, List] = None, labels: Dict[str, List] = None, load_from_file=True):
        ...
        if load_from_file is True:
            for data_type in ['train', 'val', 'test']:
                ...
                # WTF this does only keep the test data !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                # EVERY LOOP STEP OVERRIDES THE PREVIOUS ONE
    ...
    def segment(self) -> Tuple[Dict[str, list], Dict[str, list]]:
        for data_type in ['train', 'val', 'test']:
            ...
            if os.getenv('SEQ', 'False') == 'True':
                # I use sequential processing for debugging
                processor = map
            else:
                # we use multiprocessing to accelerate the split process
                cores = min(multiprocessing.cpu_count(), self.cfg.cores_used)
                pool = multiprocessing.Pool(processes=cores)
                processor = pool.starmap
            # for each subgroup in the input text find via rouge the best order of summary sentences that has the
            # biggest overlap and highest score on multiple concatenated sentences
            for i, (new_sents, new_tar) in tqdm(enumerate(processor(self.splitter.fast_rouge, tasks)),
                                                desc=f"Match {data_type}"):
                # new sents: after concatenating the summary sentences in
                #               this order the rouge score increased constantly
                # new tar: the full target summary in the original order
