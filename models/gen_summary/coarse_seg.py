...
class CoarseSegCombiner(object):
    ...
    def execute(self, split_results, count_list):
        self.split_results = split_results
        self.count_list = count_list
        self.combined_hypos = {'train': [], 'test': [], 'val': []}
        return self.combine()
        ...
        for data_type in ['train', 'val', 'test']:
            ...
            for lengths in self.count_list[data_type]:
                if start + lengths <= len(self.split_results[data_type]):
                    ...
                    # add <s> at the beginning and </s> to the end of each split segment
                    self.combined_hypos[data_type].append(
                        "<s> " + " </s> ".join(self.split_results[data_type][start:end]))
                    start = start + lengths
                else:
                    break
